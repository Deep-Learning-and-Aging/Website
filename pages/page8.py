    ...
    [
        ...
        dbc.FormGroup(
            [
                ...
                dcc.Dropdown(
                    id="Select_env_dataset_mul_ewas", options=get_dataset_options(sorted(All)), value=sorted(All)[0]
                ),
                html.Br(),
            ]
        ),
            ...
            [
                ...
                dbc.Row(
                    [
                        ...
                        dbc.Col(
                            [dcc.Loading([dcc.Graph(id="Correlation Mul - Select Ewas dataset")])],
            ...
            [
                ...
                dbc.Row(
                    [
                        ...
                        dbc.Col(
                            [dcc.Loading([dcc.Graph(id="Correlation Mul - Select Organ")])],
                            style={"overflowY": "scroll", "height": 1000, "overflowX": "scroll", "width": 1000},
                            md=9,
                        ),
                    ]
                ...
                dbc.Row(
                    [
                        ...
                        dbc.Col(
                            [
                                dcc.Loading(
                                    [html.H2(id="title_average"), dcc.Graph(id="Correlation Mul - Select Average")]
                                )
                            ],
                            style={"overflowY": "scroll", "height": 1000, "overflowX": "scroll", "width": 1000},
                            md=9,
...
@app.callback(
    ...
    [
        ...
        Input("Select_algorithm_method1", "value"),
        Input("Select_env_dataset_mul_ewas", "value"),
    ],
)
def _plot_with_given_organ_dataset(corr_type, subset_method, env_dataset):
    if corr_type is not None and subset_method is not None:
        ...
        env_matrix = pivot_table(df_env, values="corr", index=["organ_1"], columns=["organ_2"], dropna=False)
        ...
        r2_score_y = pivot_table(df_env, values="score_1", index=["organ_1"], columns=["organ_2"], dropna=False)
        r2_score_y = r2_score_y.values[order_dendrogram, :]
        r2_score_y = r2_score_y[order_dendrogram, :]
        r2_score_x = pivot_table(df_env, values="score_2", index=["organ_1"], columns=["organ_2"], dropna=False)
        r2_score_x = r2_score_x.values[order_dendrogram, :]
        r2_score_x = r2_score_x[:, order_dendrogram]
        std_y = pivot_table(df_env, values="std_1", index=["organ_1"], columns=["organ_2"], dropna=False)
        std_y = std_y.values[order_dendrogram, :]
        std_y = std_y[:, order_dendrogram]
        std_x = pivot_table(df_env, values="std_2", index=["organ_1"], columns=["organ_2"], dropna=False)
...
@app.callback(
    ...
    [
        ...
        Input("Select_algorithm_method2", "value"),
        Input("Select_organ_mul_ewas", "value"),
    ],
)
def _plot_with_given_organ_dataset(corr_type, subset_method, organ):
    if corr_type is not None and subset_method is not None:
        ...
        matrix_org = pivot_table(df_org, values="corr", index=["organ_1"], columns=["env_dataset"], dropna=False)
        r2_score_y = pivot_table(df_org, values="score_1", index=["organ_1"], columns=["env_dataset"], dropna=False)
        r2_score_x = pivot_table(df_org, values="score_env", index=["organ_1"], columns=["env_dataset"], dropna=False)
        std_y = pivot_table(df_org, values="std_1", index=["organ_1"], columns=["env_dataset"], dropna=False)
        std_x = pivot_table(df_org, values="std_env", index=["organ_1"], columns=["env_dataset"], dropna=False)
        customdata = np.dstack((r2_score_x, std_x, r2_score_y, std_y))
        hovertemplate = "Organ x : %{x}<br> Score organ x : %{customdata[0]:.3f}<br>Std organ y : %{customdata[1]:.3f}<br>Organ y : %{y}<br>Score organ y : %{customdata[2]:.3f}<br>Std organ y : %{customdata[3]:.3f}"
        try:
        ...
        d["layout"] = dict(xaxis=dict(dtick=1), yaxis=dict(dtick=1), width=800, height=800)
        return Figure(d)
    else:
        return Figure()
