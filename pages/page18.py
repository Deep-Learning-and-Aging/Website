...
if MODE == "All":
    organ_select = dbc.FormGroup(
        [
            ...
            dcc.Dropdown(id="Select_x_feat_imps_xwas", options=get_dataset_options(All)),
            html.Br(),
        ]
    )
else:
    organ_select = dbc.FormGroup(
        [
            html.P("Select X dataset : "),
            dcc.Dropdown(id="Select_x_feat_imps_xwas", options=get_dataset_options([MODE]), value=MODE),
            html.Br(),
        ],
        style={"display": "None"},
    )
    ...
    [
        dbc.FormGroup(
            [
                ...
                dcc.Dropdown(id="Select_organ_feat_imps", options=get_dataset_options(targets), value="Heart"),
                html.Br(),
            ]
        ),
        organ_select,
...
@app.callback(
    [
        ...
        Output("table_feature_imps_xwas", "columns"),
        Output("Plot Feature Imps XWAS", "figure"),
        Output("scores_xwas", "children"),
    ],
...
def _plot_r2_scores(value_x, value_organ):
    if None not in [value_x, value_organ]:
        ...
        if len(list_df) > 0:
            ...
            score_lightgbm = scores_lightgbm[
                (scores_lightgbm.env_dataset == value_x) & (scores_lightgbm.organ == value_organ)
            ].iloc[0]["r2"]
            score_nn = scores_nn[(scores_nn.env_dataset == value_x) & (scores_nn.organ == value_organ)].iloc[0]["r2"]
            score_elastic = scores_elastic[
                (scores_elastic.env_dataset == value_x) & (scores_elastic.organ == value_organ)
            ].iloc[0]["r2"]
            ...
            title = "R-Squared : ElasticNet %.3f, LightGBM %.3f, NeuralNetwork %.3f" % (
                score_elastic,
                score_lightgbm,
                score_nn,
            )
            ...
            ## Plot
            d = {
                "data": [Bar(name=model, x=df[model], y=df.index, orientation="h") for model in sorted(df.columns)],
                "layout": dict(height=len(df.index) * 20, margin={"l": 40, "b": 30, "t": 10, "r": 0}),
            }
