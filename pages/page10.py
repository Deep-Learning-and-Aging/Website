...
organs_gwas_volcano = sorted(
    list(set([elem.split("_")[3].replace(".csv", "") for elem in list_files_volcano]))
)  # ['Heart']
    ...
    [
        dbc.FormGroup(
            [
                ...
                dcc.Dropdown(
                    id="select_organ", options=get_dataset_options(organs_gwas_manhattan), value=value_manhattan
                ),
                html.Br(),
            ]
        )
...
controls2 = dbc.Card(
    [
        dbc.FormGroup(
            [
                html.P("Select an organ: "),
                dcc.Dropdown(id="select_organ2", options=get_dataset_options(organs_gwas_volcano), value=value_volcano),
                html.Br(),
            ]
        )
    ]
            ...
            [
                ...
                dbc.Row(
                    [
                        ...
                        dbc.Col(
                            [dcc.Loading([dcc.Graph(id="vol_plot")])],
...
def _plot_volcano_plot(organ):
    if organ is not None:
        ...
        if organ != "All":
            df = load_csv(filename_volcano + organ + "_withGenes.csv")[
                ["CHR", "Gene", "Gene_type", "SNP", "P_BOLT_LMM_INF", "BETA"]
            ].sort_values("CHR")
            d["data"] = [
                Scatter(
                    x=[df["BETA"].min() - df["BETA"].std(), df["BETA"].max() + df["BETA"].std()],
            ...
            d["data"] += [
                Scatter(
                    x=df[df.CHR == chromo]["BETA"],
                    y=-np.log(df[df.CHR == chromo]["P_BOLT_LMM_INF"]),
                    mode="markers",
                    name="CHR %s" % chromo,
                    marker=dict(color=dict_chr_to_colors[str(chromo)]),
                    customdata=np.stack(
                        (df[df.CHR == chromo]["SNP"], df[df.CHR == chromo]["Gene"], df[df.CHR == chromo]["Gene_type"]),
                        axis=1,
                    ),
            ...
            df = load_csv(filename_volcano + organ + "_withGenes.csv")[
                ["CHR", "Gene", "Gene_type", "SNP", "P_BOLT_LMM_INF", "BETA", "organ"]
            ].sort_values("CHR")
            d["data"] = [
                Scatter(
                    x=[df["BETA"].min() - df["BETA"].std(), df["BETA"].max() + df["BETA"].std()],
            ...
            d["data"] += [
                Scatter(
                    ...
                    customdata=np.stack(
                        (
                            df[df.CHR == chromo]["SNP"],
                            df[df.CHR == chromo]["organ"],
                            df[df.CHR == chromo]["Gene"],
                            df[df.CHR == chromo]["Gene_type"],
                        ),
                        axis=1,
                    ),
                    hovertemplate="""SNP : %{customdata[0]}
