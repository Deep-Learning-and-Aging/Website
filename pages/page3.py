...
path_score_scalar = "page2_predictions/Performances/PERFORMANCES_tuned_alphabetical_instances_Age_test.csv"
...
if MODE == "All":
    organ_select = dbc.FormGroup(
        [
            ...
            dcc.Dropdown(id="Select_organ_1", options=get_dataset_options(hierarchy_biomarkers.keys())),
            html.Br(),
        ]
    )
else:
    organ_select = dbc.FormGroup(
        [
            html.P("Select an Organ : "),
            dcc.Dropdown(id="Select_organ_1", options=get_dataset_options([MODE]), value=MODE),
            html.Br(),
        ],
        style={"display": "None"},
    )
    ...
    [
        ...
        dbc.FormGroup(
            [
                ...
                dcc.Dropdown(id="Select_view_1", options=get_dataset_options([])),
                html.Br(),
            ]
        ),
        dbc.FormGroup(
            [
                ...
                dcc.Dropdown(id="Select_transf_1", options=get_dataset_options([])),
                html.Br(),
            ]
        ),
...
table_df = DataFrame(
    data={
        "Corr": ["Correlation", "ElasticNet", "LightGBM", "NeuralNetwork"],
        "Correlation": [1, 0, 0, 0],
        "ElasticNet": [0, 1, 0, 0],
        "LightGBM": [0, 0, 1, 0],
...
@app.callback(
    [
        ...
        Output("table_feature_imps", "columns"),
        Output("Plot Feature Imps", "figure"),
        Output("scores", "children"),
    ],
    [
        Input("select_target", "value"),
        Input("Select_organ_1", "value"),
        Input("Select_view_1", "value"),
        Input("Select_transf_1", "value"),
    ],
)
def _plot_r2_scores(value_target, value_organ, value_view, value_transformation):
    if value_transformation is not None:
        ...
        score_model = score[
            (score["organ"] == value_organ)
            & (score["view"] == value_view)
            & (score["transformation"] == value_transformation)
        ...
        title = (
            "R2 = %.3f (ElasticNet), R2 = %.3f (LightGBM), R2 = %.3f (NeuralNetwork), Sample Size = %d (Scalars)"
            % (score_elasticnet, score_lightgbm, score_nn, sample_size)
        )
        list_models = []
        list_df = list_obj(
            path_feat_imps + "FeatureImp_%s_%s_%s_%s_" % (value_target, value_organ, value_view, value_transformation)
        )
        list_df_sd = list_obj(
            path_feat_imps
            + "FeatureImp_sd_%s_%s_%s_%s_" % (value_target, value_organ, value_view, value_transformation)
        )
        ...
        d = {
            "data": [
                Bar(
                    ...
                    y=df_abs.index,
                    orientation="h",
                    hovertemplate="Feature Name: %{y}<br>Signed Feature importance : %{customdata:.3f}",
                    customdata=df[model].reindex(df_abs.index),
                )
                for model in sorted(df_abs.columns)
            ],
            "layout": dict(
                height=len(df.index) * 20, margin={"l": 40, "b": 30, "t": 10, "r": 0}, xaxis_title="Feature importance"
            ),
        }
        ...
        return (
            df_str.iloc[::-1].to_dict(),
            df_abs.iloc[::-1].to_dict(),
            [{"name": i, "id": i} for i in ["Features"] + sorted(df.columns)],
            Figure(d),
            title,
        )
    else:
