...
def _display_gif(organ, view, transformation, sex, age_group, sample):
    if None not in [organ, view, transformation, sex, age_group, sample]:
        df = load_csv(
            "page12_AttentionMapsVideos/RawVideos/files/"
            + "AttentionMaps-samples_Age_Heart_MRI_%s.csv" % transformation
        )
        df = df[
            (df["age_category"] == age_group.lower())
            & (df["Sex"] == sex)
            & (df["sample"] == int(sample))
            & (df["aging_rate"] == "normal")
        ]
        title = "Chronological Age = %.3f" % df["Age"]
        path_to_gif = path_gif + "%s/%s/%s/RawVideo_Age_Heart_MRI_%s_%s_%s_%s." % (
            transformation,
            dict_sex_id_to_sex[sex],
            age_group.lower(),
            transformation,
            dict_sex_id_to_sex[sex],
            age_group.lower(),
