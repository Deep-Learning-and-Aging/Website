    ...
    [
        ...
        dbc.Row(
            [
                ...
                dbc.Col(
                    [dcc.Loading([dcc.Graph(id="Plot R2 Heatmap")])], style={"overflowX": "scroll", "width": 1000}, md=9
                ),
            ]
        ),
        dbc.Row([dbc.Col([dcc.Graph(id="Plot Bar Plot")], style={"overflowX": "scroll", "width": 1000})]),
    ],
    fluid=True,
)
...
def _compute_plots(algo, group, view_type):
    if algo is not None and step is not None:
        if algo != "Best Algorithm":
            df = load_csv(
                path_scores_ewas + "Scores_%s_%s.csv" % (algo, dict_step_to_proper[step]),
                usecols=["env_dataset", "r2", "std", "organ", "subset", "sample_size"],
            )
            ...
            for algo_ in ["LightGbm", "NeuralNetwork", "ElasticNet"]:
                df_algo = load_csv(
                    path_scores_ewas + "Scores_%s_%s.csv" % (algo_, dict_step_to_proper[step]),
                    usecols=["env_dataset", "r2", "std", "organ", "subset", "sample_size"],
                )
                df_algo["algo"] = algo_
        ...
        if view_type == "Organ":
            d2["data"] = [
                Bar(
                    name=organ,
                    x=df[df.organ == organ].env_dataset,
                    y=df[df.organ == organ].r2,
                    error_y=dict(type="data", array=df[df.organ == organ]["std"]),
                )
                for organ in df.organ.drop_duplicates()
            ]
        elif view_type == "X":
            d2["data"] = [
                Bar(
                    name=x_dataset,
                    x=df[df.env_dataset == x_dataset].organ,
                    y=df[df.env_dataset == x_dataset].r2,
                    error_y=dict(type="data", array=df[df.env_dataset == x_dataset]["std"]),
                )
                for x_dataset in df.env_dataset.drop_duplicates()
            ]
